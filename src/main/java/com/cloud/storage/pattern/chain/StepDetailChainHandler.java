...
//        判断是否是详细步数
        if (PropertiesReader.getProp(" DATATYPE_STEPDETIAL").equalsIgnoreCase(dataType)) {
            ...
            Log.info(" data deal by StepDetailChainHandler !");
...
//                insert into database
                boolean bool = context.getObservationService().
                        insertOrUpdateData(data.getPhone(), measureTime,
                                PropertiesReader.getProp("DATATYPE_STEPDETIAL"),
                                PropertiesReader.getProp("APPTYPE_AppA"),
                                JSONObject.fromObject(map).toString(), data.getCollectDate());
                if (!bool)
                    Log.error("StepDetailChainHandler save data into db error ! ");
                return bool;
            } else {
                Log.error(" StepDetailChainHandler datavalue is null !");
                return false;
            }
        } else {
            if (successor != null)
